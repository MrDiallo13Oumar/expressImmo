...
export class ListContratComponent {
  displayedColumns: string[] = ['id', 'reservation_nom','propriete_reference' ,'statut', 'action'];
        ...
        if (result?.event && result.event === 'insert') {
          ...
          this.service.create('contrat', 'create.php', formData).subscribe({
            next: (response) => {
              ...
              const message =
                response?.message || 'Contrat enregistré avec succès !';
...
modify_by = localStorage.getItem('id_user');
  ...
  liberer(_ID: any, statut: string) {
    ...
    this.service.create('public', 'update.php', formData).subscribe({
      next: (response) => {
        const message = response?.message || 'Proprietete Liberer avec Succes!';
